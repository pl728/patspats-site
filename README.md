# patspats.xyz

Static source for Patrick Lin's personal site and SkySwipe's AT Protocol OAuth metadata.

## Routes

- `/` — personal homepage
- `/projects/` — apps and software projects
- `/learning/` — current study materials and learning links
- `/learning/method/` — Patrick's learning methodology
- `/files/mastering-java-chapter-1.pdf` — Mastering Java reader edition, currently Chapters 1–3
- `/files/mastering-javascript-and-typescript.pdf` — Mastering JavaScript and TypeScript reader edition
- `/skyswipe/` — SkySwipe landing page
- `/skyswipe/oauth/client-metadata.json` — SkySwipe native OAuth client metadata

## Cloudflare Pages

- Framework preset: `None`
- Build command: leave blank
- Build output directory: `.`
- Production branch: `main`

The custom domain is `patspats.xyz`.
